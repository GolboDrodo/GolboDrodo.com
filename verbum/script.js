...
document.addEventListener('DOMContentLoaded', () => {
       // 1. Inizializzazione Cartelle, Colori e UI
    loadColors();
    
    const isFirstRun = !localStorage.getItem('verbum_folders');
    foldersList = JSON.parse(localStorage.getItem('verbum_folders') || '[]');

    if (isFirstRun) {
        const defaultNoteId = Date.now();
        const defaultContent = `Benvenuto in Verbum.

Questo documento contiene le istruzioni per utilizzare al meglio il sistema, le scorciatoie da tastiera e le funzioni principali.
Se lo desideri, puoi eliminare questo file trascinandolo nel cestino o tramite il menu contestuale (tasto destro).


1. INTERFACCIA E ICONE BASE

* SCRIVI:   Apre l'editor di testo per creare nuove note.
* ARCHIVIO: Contiene l'elenco di tutte le note di testo (.txt).
* CESTINO:  Raccoglie i file eliminati. Può essere svuotato.
* WEBCAM:   Permette di scattare foto in tempo reale.
* GALLERIA: Raccoglie tutti gli scatti fotografici (.png) salvati.
* TETRIS:   Apre l'applicazione di gioco.


2. SCORCIATOIE DA TASTIERA

* TASTO [ESC]: Chiude istantaneamente la finestra attiva.
* TASTO [ENTER]: Conferma i prompt (salvataggio) o scatta una foto.
* [FRECCE DESTRA/SINISTRA]: Scorre le foto nella Galleria.


3. SELEZIONI E INTERAZIONI

* CLICK SINGOLO: Seleziona un oggetto.
* [CTRL/CMD + CLICK]: Selezione multipla.
* [SHIFT + CLICK]: Selezione a intervallo nelle liste.
* CLICK E TRASCINAMENTO : Crea un rettangolo di selezione multipla.


4. MENU CONTESTUALE (TASTO DESTRO)

* SU DESKTOP: Incolla, Nuova Cartella, Colori.
* SU ICONE: Copia, Rinomina, Aggiungi ai preferiti, Raggruppa in cartella, Elimina.


5. GESTIONE FILE E DRAG & DROP

* Trascinando i file dall'Archivio o Galleria al Desktop, ne crei una copia.
* Puoi trascinare gli ogetti l'uno dentro l'altro per raggrupparli rapidamente in una nuova cartella


6. SICUREZZA, SALVATAGGIO E PRIVACY

* SALVATAGGIO LOCALE: Tutti i file, le cartelle e le impostazioni di colore vengono salvati istantaneamente nella memoria interna del tuo browser (tecnologie IndexedDB e LocalStorage).
* PERSISTENZA DEI DATI: I tuoi file non verranno persi o cancellati automaticamente. 
Rimarranno memorizzati in modo permanente anche  se chiudi la scheda o spegni il computer.
* 100% OFFLINE: Verbum risiede interamente sul tuo dispositivo. Nessun dato viene inviato a server esterni o condiviso via Internet. 
Tutto ciò che crei è privato e visibile solo a te.
`;

        // Salva il file vero e proprio nel database (IndexedDB)
        const defaultNote = {
            id: defaultNoteId,
            name: "manuale_utente.txt",
            content: defaultContent,
            fav: false,
            trashed: false,
            hiddenFromApp: false
        };
        dbOp("readwrite", s => s.put(defaultNote));
        ...
        const defaultShortcut = {
            ...
            parentId: null, 
            top: '80px',    
            left: '520px' // Si posiziona perfettamente a destra dell'icona Tetris!
        };

        foldersList.push(defaultShortcut);
    ...
    // Finestra Colori
        bind('menu-colors', 'click', () => { 
        hideContextMenus(); 
        const win = document.getElementById('window-colori');
        win.style.left = contextMenuX + 'px';
        win.style.top = contextMenuY + 'px';
        openWindow('window-colori'); 
    });
    bind('btn-close-colori', 'click', () => closeWindow('window-colori'));
    bind('btn-reset-colori', 'click', resetColors);
    
    ['bg', 'txt', 'win'].forEach(type => {
        const input = document.getElementById('color-' + type);
        if (input) {
            input.addEventListener('input', (e) => {
                document.documentElement.style.setProperty('--color-' + type, e.target.value);
                localStorage.setItem('verbum_color_' + type, e.target.value);
            });
        }
    });
...
function createFolderDOMElement(fData, parentContainer, isNested) {
    ...
    if (fData.type === 'photo') {
        ...
        const hasChildren = foldersList.some(f => f.parentId === fData.id);
        folder.innerHTML = `
            <div id="fav-${fData.id}" class="fav-indicator ${fData.fav ? 'visible' : ''}">:-)</div>
            <div class="icon-img icon-folder ${hasChildren ? 'full' : ''}"></div>
            <span class="icon-label">${fData.name}</span>
        `;
    }
...
function loadColors() {
    const bg = localStorage.getItem('verbum_color_bg') || '#0000FF';
    const txt = localStorage.getItem('verbum_color_txt') || '#FFFFFF';
    const win = localStorage.getItem('verbum_color_win') || '#ff53dd';
    document.documentElement.style.setProperty('--color-bg', bg);
    document.documentElement.style.setProperty('--color-txt', txt);
    document.documentElement.style.setProperty('--color-win', win);
    if (document.getElementById('color-bg')) document.getElementById('color-bg').value = bg;
    if (document.getElementById('color-txt')) document.getElementById('color-txt').value = txt;
    if (document.getElementById('color-win')) document.getElementById('color-win').value = win;
}

function resetColors() {
    localStorage.removeItem('verbum_color_bg');
    localStorage.removeItem('verbum_color_txt');
    localStorage.removeItem('verbum_color_win');
    loadColors();
}